  ...
  methods: {
    ...
    handlePublishMore() {
      ...
      for (let index = 0; index < this.selectedRowKeys.length; index++) {
        let element = this.selectedRowKeys[index]
        elements +=element+",";
      }
      postApi.updateStatus(elements, 'PUBLISHED').then(response => {
        this.$log.debug(`postId: ${elements}, status: PUBLISHED`)
    ...
    handleRecycleMore() {
      ...
      postApi.updateStatus(elements, 'RECYCLE').then(response => {
        this.$log.debug(`postId: ${elements}, status: RECYCLE`)
        this.selectedRowKeys = []
        this.loadPosts()
      })
    },
    handleDeleteMore() {
      ...
      postApi.delete(elements).then(response => {
        this.$log.debug(`delete: ${elements}`)
        this.selectedRowKeys = []
        this.loadPosts()
      })
    },
